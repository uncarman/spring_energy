function TFWebSocket(ipAdr,port){
    var that = this;
    //    检测浏览器是否支持WebSocket
    if (!window.WebSocket) {
    ...
    if (window.WebSocket) {
        ...
        this.socket.onerror = function(event) {
            ...
            that.socket = new WebSocket('ws://'+ipAdr+':'+port);
        };
    } else {
...
TFWebSocket.prototype.send=function(message, callback){
...
TFWebSocket.prototype.close=function(){
    this.socket.close();
};
